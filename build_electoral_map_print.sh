...
cat <(curl -s $ELEX_S3_URL/json/results-national-ap-latest.json) | \
  ...
  ndjson-map '{"statepostal": d.statepostal, "votes": d.votes.filter(obj => obj.name != "").sort((a, b) => b.votecount - a.votecount)}' | \
  ndjson-map '{"statepostal": d.statepostal, "leader": d.votes.every(o => o.votecount == 0) ? "no-votes" : d.votes[0].votecount != d.votes[1].votecount ? d.votes[0].name : "even", "winner_or_leading": d.votes.every(o => o.votecount == 0) ? "no-votes" : d.votes[0].votecount != d.votes[1].votecount && d.votes[0].bool_winner == true ? d.votes[0].name + "_winner" : d.votes[0].votecount != d.votes[1].votecount && d.votes[0].bool_winner == false ? d.votes[0].name + "_leader" : "even", "winner_margin": (d.votes[0].votepct - d.votes[1].votepct).toFixed(2)}' | \
