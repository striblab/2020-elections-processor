...
LATEST_FILE=json/uncontested-ap.json

TMPFILE=$(mktemp "/tmp/uncontested-$download_datetime.json.XXXXXXX")
...
if [ $FIRST_LEVEL == '"state"' ]; then
  ...
  if cmp --silent $TMPFILE $LATEST_FILE; then
     ...
     cp $TMPFILE "json/uncontested-$download_datetime.json"
  fi
