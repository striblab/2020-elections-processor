...
# OVERRIDE statewide API KY FOR ELEX
AP_API_KEY=$AP_API_KEY_STATE_LEVEL
...
LATEST_FILE=json/results-statewide-ap-latest.json

TMPFILE=$(mktemp "/tmp/results-statewide-ap-$download_datetime.json.XXXXXXX")

printf "\n\n\n"

printf "Starting AP statewide update ...\n\n"
...
if [ $FIRST_LEVEL == '"state"' ]; then
  ...
  if cmp --silent $TMPFILE $LATEST_FILE; then
     ...
     gzip -vc $TMPFILE | aws s3 cp - "s3://$ELEX_S3_URL/json/versions/results-statewide-ap-$download_datetime.json" \
     --profile $AWS_PROFILE_NAME \
     --acl public-read \
     --content-type=application/json \
     --content-encoding gzip
     ...
     cp $TMPFILE "json/results-statewide-ap-$download_datetime.json"
  fi
