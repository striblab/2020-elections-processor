...
aws s3 cp print/$PREZ_ELECTORAL_FILE "s3://$S3_PRINT_BUCKET/$PREZ_ELECTORAL_FILE" \
...
aws s3 cp print/$PREZ_ELECTORAL_MAP "s3://$S3_PRINT_BUCKET/$PREZ_ELECTORAL_MAP" \
--profile $PRINT_AWS_PROFILE_NAME \
--acl public-read \
...
aws s3 cp print/$PREZ_BY_COUNTY_FILE "s3://$S3_PRINT_BUCKET/$PREZ_BY_COUNTY_FILE" \
--profile $PRINT_AWS_PROFILE_NAME \
--acl public-read \
--content-type=text/csv
...
aws s3 cp print/$SENATE_BY_COUNTY_FILE "s3://$S3_PRINT_BUCKET/$SENATE_BY_COUNTY_FILE" \
--profile $PRINT_AWS_PROFILE_NAME \
--acl public-read \
--content-type=text/csv
